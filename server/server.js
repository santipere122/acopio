...
const sequelize = new Sequelize('acopio', 'nodeapp', 'PwdSegura123', {
  host: 'localhost',
  port: 3306,
  dialect: 'mysql',
  dialectOptions: {
...
sequelize.authenticate()
  .then(() => console.log('✔ Conectado a MySQL como nodeapp'))
...
app.post('/api/usuarios/crear', async (req, res) => {
  const { Usuario: nombreUsuario, Password, id_chofer } = req.body;

  if (!nombreUsuario || !Password) {
      return res.status(400).json({ error: 'Faltan datos necesarios' });
  }

  const rol = id_chofer ? 'Chofer' : 'Admin';
  ...
  try {
      const nuevoUsuario = await Usuario.create({
          ...
          Rol: rol,  // Añadido el campo Rol
          Fecha_Creacion: new Date(),
          Fecha_Modificaion: new Date()
      });
...
app.put('/api/usuarios/editar/:id', async (req, res) => {
  ...
  try {
    ...
    usuario.id_chofer = id_chofer !== null ? id_chofer : null;
...
app.get('/api/usuarios/verificar/:nombre', async (req, res) => {
  const { nombre } = req.params;

  try {
      const usuario = await Usuario.findOne({ where: { Usuario: nombre } });
...
// Endpoint para verificar si hya un chofer asociado al usuario
app.get('/api/usuarios/verificar-chofer/:id_chofer', (req, res) => {
  const id_chofer = req.params.id_chofer;

  // Suponiendo que tienes un modelo de Usuario
  Usuario.findOne({ where: { id_chofer: id_chofer } })
      .then(usuario => {
          if (usuario) {
...
app.get('/api/choferes',(req,res)=>{
 ...
 app.post('/api/choferes/crear', async (req, res) => {
  try {
    ...
    console.log('Chofer creado:', nuevoChofer);
    res.status(201).json(nuevoChofer);
  } catch (error) {
    console.error('Error al crear chofer:', error);
    res.status(500).json({ mensaje: 'Error al crear chofer', error });
  }
});
...
// Reasignar Chofer y Camión del Acopio
app.post('/api/acopios/reasignar', async (req, res) => {
  const { acopioId, chofer_nuevo, camion_nuevo } = req.body;

  const query = `
    CALL ReasignarChoferCamionAcopio(?, ?, ?);
  `;

  try {
    const results = await sequelize.query(query, {
      replacements: [acopioId, chofer_nuevo, camion_nuevo]
    });

    res.json({ message: 'Chofer y camión reasignados con éxito', acopio: results[0] });
  } catch (error) {
    console.error('Error al reasignar chofer y camión:', error);
    res.status(500).json({ error: 'Error al reasignar chofer y camión' });
  }
});
...
app.post('/api/acopios/completar', async (req, res) => {
  const { id_acopio, monto_pagar, fecha, cantidad } = req.body;
  ...
  try {
    const results = await sequelize.query(query, {
      replacements: [id_acopio, monto_pagar, fecha, cantidad]
    });
...
app.get('/api/geocode', async (req, res) => {
  ...
  if (!address) {
    return res.status(400).json({ error: 'Debe enviar el parámetro address' });
  }
  ...
  try {
    ...
    const response = await axios.get(nominatimUrl, {
      headers: { 'User-Agent': 'MiAppAcopios/1.0 (tu-email@dominio.com)' }
    });
...
// Geocodificación inversa: coordenadas → dirección //por si se necesita
app.get('/api/reverse-geocode', async (req, res) => {
  const { lat, lng } = req.query;
  if (!lat || !lng) {
    return res.status(400).json({ error: 'Debe enviar lat y lng' });
  }
